#!/usr/bin/env python3
"""Deprecated compatibility wrapper. Use ``coinfosim scenario run support2``.

The SUPPORT2 180-day mortality three-arm scenario now lives in the
installable ``coinfosim`` package (see
``coinfosim.scenarios.definitions.support2`` and
...
by this wrapper. Custom classifier selection (``--classifiers``,
``--rf-calibration-file`` in the previous version of this script) is not
part of the public CLI surface; build a custom
``DatasetAnchoredExecutionSpec`` via
``coinfosim.scenarios.definitions.support2`` directly if you need it.
"""
...
SCENARIO_SLUG = "support2"
DEFAULT_RAW_DIR = "data/raw/support2"

_DEPRECATION_NOTICE = (
    "scripts/run_support2_scenario.py is deprecated and will be removed in a "
