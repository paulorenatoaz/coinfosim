#!/usr/bin/env python3
"""Deprecated compatibility wrapper. Use ``coinfosim scenario run air-quality``.

The UCI Air Quality three-arm scenario now lives in the installable
``coinfosim`` package (see ``coinfosim.scenarios.definitions.air_quality``
and ``coinfosim.scenarios.service``). This script is kept only so existing
...
SCENARIO_SLUG = "air-quality"
DEFAULT_RAW_DIR = "data/raw/air_quality"

_DEPRECATION_NOTICE = (
    "scripts/run_air_quality_scenario.py is deprecated and will be removed in "
