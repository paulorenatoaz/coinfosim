...
\begin{frame}{Proveniência e integridade experimental}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.24\textwidth}
      \begin{beamercolorbox}[wd=\linewidth,sep=.12cm,rounded=true]{block body}
        ...
        {\fontsize{8.0}{9.5}\selectfont Dados reais de treinamento separados do teste real fixo.}
      \end{beamercolorbox}
    \end{column}
    \begin{column}{0.24\textwidth}
      \begin{beamercolorbox}[wd=\linewidth,sep=.12cm,rounded=true]{block body}
  ...
  \speakernotes
    {Em Ciência de Dados, proveniência não significa apenas saber de qual site veio um arquivo. Ela descreve toda a trajetória: qual arquivo canônico foi usado, como sua integridade foi verificada, como o alvo e a coorte foram definidos, como as linhas foram particionadas e em que etapa cada transformação foi estimada. No CoInfoSim, os papéis de dados reais de treinamento e teste real fixo são explícitos. Padronização e ajuste dos modelos geradores usam somente o treinamento; o teste fica reservado para avaliação comum dos três braços. Os hashes dos arquivos e as impressões digitais das partições ajudam a detectar trocas silenciosas de dados.}
...
\begin{frame}{Resultados adicionais — Occupancy e Air Quality}
  ...
  {\fontsize{7.3}{8.7}\selectfont\color{CoInfoGray}
    Os quatro indicadores foram recalculados diretamente pelo código atual a partir dos resultados brutos por replicação regenerados nesta revisão (cap.\ 7). Nenhum valor deriva do antigo $N^\star$.}
...
\begin{frame}{Resultados adicionais — SUPPORT2}
  \eyebrow{Escala completa ($n=512$): quatro indicadores recalculados diretamente pelo código atual}
  \vspace{0.05cm}
  \centering
  \renewcommand{\arraystretch}{1.0}
  ...
  \vspace{0.05cm}
  \eyebrow{Recalculado pelo código atual: execução smoke, $n\leq32$}
  \vspace{0.02cm}
  ...
  \begin{tabular}{lcccc}
    ...
    Classificador, treinamento $\rightarrow$ teste & $\rankrho$ & \AW{} & $A_R$ & $S_R$ \\
    \midrule
    Linear SVM, Gaussiana única $\rightarrow$ Real & 0,011 & 0,504 & 0,728 & 0,728 \\
    Linear SVM, GMM $\rightarrow$ Real             & 0,271 & 0,591 & 0,794 & 0,689 \\
    Random Forest, Gaussiana única $\rightarrow$ Real & 0,624 & 0,731 & 0,479 & 0,882 \\
    Random Forest, GMM $\rightarrow$ Real             & 0,533 & 0,687 & 0,467 & 0,879 \\
    \bottomrule
  \end{tabular}

  \vspace{0.02cm}
  {\fontsize{6.6}{7.8}\selectfont\color{CoInfoGray}
    Bloco inferior: escala reduzida (\texttt{scenario\_run\_id=0}), não comparável linha a linha ao superior; reproduz o mesmo padrão de $A_R$/$S_R$ do Random Forest.\par}
  \sourcecite{SUPPORT2 000007/000008 e execução smoke; indicadores recalculados pelo código atual; cap.\ 7 e apênd.\ A/B.}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.43\textwidth}
      ...
      {\fontsize{8.7}{10.4}\selectfont
        ...
        {\fontsize{7.0}{8.4}\selectfont\texttt{f7cfb0b3}}\par\medskip
        {\fontsize{6.6}{8.0}\selectfont\color{CoInfoGray}(revisão editorial de perfis de cooperação preditiva, branch \texttt{feature/academic-report-revision})}\par\medskip
