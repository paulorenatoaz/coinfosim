...
\begin{frame}{Definições formais dos quatro indicadores}
  ...
  \vspace{.10cm}
  \begin{beamercolorbox}[wd=\linewidth,sep=.11cm,rounded=true]{block body}
    {\bfseries\color{CoInfoBlue}Concordância de existência ($A_R$) e similaridade amostral ($S_R$) --- nunca multiplicadas}\par\vspace{.05cm}
    \begin{columns}[T,onlytextwidth]
      \begin{column}{0.48\textwidth}
        ...
        {\fontsize{8.2}{9.7}\selectfont $\mathcal C_g(n_t)$: pares com $R^{(g)}$ definido. União vazia: $A_R=1$ (ausência em ambos, não posições coincidentes).}
      \end{column}
      \begin{column}{0.49\textwidth}
        \resizebox{0.92\linewidth}{!}{$
        ...
        {\fontsize{8.0}{9.4}\selectfont Definida só no suporte compartilhado; sem pares compartilhados: indisponível, nunca 1.}
      \end{column}
    \end{columns}
  \end{beamercolorbox}
...
\begin{frame}{Resultados adicionais — Occupancy e Air Quality}
  \eyebrow{Indicadores herdados em $n=512$; $A_R$/$S_R$ indisponíveis nesta revisão}
  \vspace{0.06cm}
  \centering
  ...
  \begin{tabular}{llcccc}
    ...
    \multirow{6}{*}{\shortstack[l]{Occupancy}}
      & SVM, Gaussiana única $\rightarrow$ Real & 0,9528 & 0,9204 & --- & --- \\
      & SVM, GMM $\rightarrow$ Real & 0,8774 & 0,8624 & --- & --- \\
      & Reg. Logística, Gaussiana única $\rightarrow$ Real & 0,9036 & 0,8839 & --- & --- \\
      & Reg. Logística, GMM $\rightarrow$ Real & 0,8738 & 0,8903 & --- & --- \\
      & Gaussian NB, Gaussiana única $\rightarrow$ Real & 0,9976 & 0,9892 & --- & --- \\
      & Gaussian NB, GMM $\rightarrow$ Real & 0,9996 & 0,9978 & --- & --- \\
    \midrule
    \multirow{6}{*}{\shortstack[l]{Air Quality}}
      & SVM, Gaussiana única $\rightarrow$ Real & 0,8944 & 0,8882 & --- & --- \\
      & SVM, GMM $\rightarrow$ Real & 0,9580 & 0,9333 & --- & --- \\
      & Reg. Logística, Gaussiana única $\rightarrow$ Real & 0,9359 & 0,9118 & --- & --- \\
      & Reg. Logística, GMM $\rightarrow$ Real & 0,9839 & 0,9613 & --- & --- \\
      & Gaussian NB, Gaussiana única $\rightarrow$ Real & 1,0000 & 1,0000 & --- & --- \\
      & Gaussian NB, GMM $\rightarrow$ Real & 0,9996 & 0,9978 & --- & --- \\
    \bottomrule
  \end{tabular}
  ...
  {\fontsize{7.9}{9.4}\selectfont\color{CoInfoGray}
    $\rankrho$ e $\AW{}$ são herdados dos relatórios publicados dos cenários Occupancy 000002 e Air Quality 000005/000006 (esquema anterior; ver Seção de disponibilidade de dados do relatório final, capítulo 7). $A_R$ e $S_R$ exigiriam os resultados brutos por réplica dessas execuções, não persistidos localmente nesta revisão --- por isso aparecem como travessão, nunca como zero, e não como o antigo valor de similaridade de $N^\star$, que media outro objeto.}
...
\begin{frame}{Resultados adicionais — SUPPORT2}
  \eyebrow{Herdados (full, $n=512$): $\rankrho$/\AW{} apenas; $A_R$/$S_R$ indisponíveis}
  \vspace{0.05cm}
  \centering
  \renewcommand{\arraystretch}{1.0}
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    SVM, Gaussiana única $\rightarrow$ Real & 0,8672 & 0,8614 & --- & --- \\
    SVM, GMM $\rightarrow$ Real & 0,9708 & 0,9344 & --- & --- \\
    Reg. Log., Gaussiana única $\rightarrow$ Real & 0,9853 & 0,9514 & --- & --- \\
    Reg. Log., GMM $\rightarrow$ Real & 0,9827 & 0,9461 & --- & --- \\
    Gaussian NB, Gaussiana única $\rightarrow$ Real & 0,7915 & 0,7951 & --- & --- \\
    Gaussian NB, GMM $\rightarrow$ Real & 0,9144 & 0,8715 & --- & --- \\
    Random Forest, Gaussiana única $\rightarrow$ Real & 0,8387 & 0,8282 & --- & --- \\
    Random Forest, GMM $\rightarrow$ Real & 0,8665 & 0,8429 & --- & --- \\
    \bottomrule
  \end{tabular}
  ...
  {\fontsize{7.6}{9.1}\selectfont\color{CoInfoGray}
    Bloco superior: valores herdados do esquema anterior, cenários SUPPORT2 000007/000008 ($n=512$); $A_R$/$S_R$ exigiriam os dados brutos por réplica, não persistidos localmente. Bloco inferior: \texttt{scenario\_run\_id=0}, \texttt{output/validation/support2-random-forest-smoke/}, indicadores calculados diretamente por \texttt{coinfosim.results.structural} --- escala reduzida, não comparável linha a linha ao bloco superior.}
  \sourcecite{cenários SUPPORT2 000007 e 000008 (herdados); execução smoke local (recalculada); capítulo 7 e apêndices A/B do relatório final.}
\end{frame}
    ...
    \begin{column}{0.32\textwidth}
      \begin{beamercolorbox}[wd=\linewidth,sep=.17cm,rounded=true]{block body}
        ...
        \begin{itemize}\setlength{\itemsep}{.03cm}
          ...
          \item \AW{} ignora magnitude; pares indefinidos são ignorados, não discordantes;
          \item $\mathbf R$ guarda só a última inversão de cada par; $A_R$ ignora direção e magnitude; $S_R$ só existe no suporte compartilhado;
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.43\textwidth}
      ...
      {\fontsize{8.7}{10.4}\selectfont
        ...
        {\fontsize{7.0}{8.4}\selectfont\texttt{ce1bafd83a34fc3aeef504ab450fa8d75dbcaff3}}\par\medskip
        {\fontsize{6.6}{8.0}\selectfont\color{CoInfoGray}(base da revisão de perfis de cooperação preditiva; ver apêndice B do relatório final para o commit desta revisão editorial)}\par\medskip
