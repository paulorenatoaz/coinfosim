...
Assim como os quatro cenários de escala completa da Seção~\ref{ap:resultados-cenarios}, a execução \textit{smoke} do SUPPORT2 usada nas Seções~\ref{sec:reversal-matrix} e~\ref{sec:support2-rf} tem seus resultados brutos por replicação persistidos no ambiente auditado (fora do controle de versão, como todo o diretório \texttt{output/}, Apêndice~B). Por ser a execução mais compacta, seu caminho é listado individualmente nesta seção como referência mínima de reprodução; os caminhos dos quatro cenários de escala completa estão listados nas seções acima, sob \texttt{output/reports/simulations/}:
...
Os relatórios online listados na Seção~\ref{ap:resultados-cenarios} são a fonte exaustiva dos resultados desta revisão, incluindo as matrizes $\Wmat$/$\Rmat$ completas de cada cenário. O PDF não reproduz todas as matrizes e curvas porque isso prejudicaria a leitura e duplicaria centenas de visualizações; ele apresenta uma seleção orientada pelas hipóteses científicas, com os quatro indicadores tabulados por completo. A rastreabilidade é preservada por meio dos IDs imutáveis de cenário e simulação, nomes de arquivos, parâmetros registrados e resultados JSON persistidos no ambiente auditado (Seção~\ref{ap:support2-smoke}).

Os gráficos vetoriais de $\rankrho$/$\AW$ reconstruídos a partir de tabelas finais publicadas anteriormente para Occupancy e Air Quality (identificados individualmente em \texttt{figures/FIGURE\_SOURCES.md} e nas fontes de cada figura no corpo do texto) foram incorporados sem recortes que alterassem eixos, ordem dos subconjuntos, escala ou legenda; os valores de $\rankrho$/$\AW$ que ilustram foram confirmados computacionalmente nesta revisão a partir dos resultados brutos regenerados, dentro da precisão de transcrição das versões anteriores (Seção~\ref{sec:resultados-guia}). As matrizes $\Wmat$/$\Rmat$ e os gráficos de métricas do SUPPORT2 \textit{smoke} apresentados nesta revisão, em contraste, foram gerados diretamente pelas funções vigentes de \texttt{coinfosim.reports.predictive\_profile\_visualization} sobre os dados brutos da Seção~\ref{ap:support2-smoke}. Os valores de $\AR$, $\DR$ e $\SR$ apresentados em todas as tabelas do Capítulo~7, para os quatro cenários de escala completa e para o exemplo \textit{smoke}, foram computados diretamente pelo código atual sobre resultados brutos por replicação, sem reconstrução a partir de nenhuma tabela ou figura publicada anteriormente --- esses três indicadores não existiam sob o esquema anterior de $\NstarHistorical$.
