...
O corpo do relatório (Capítulo~4) mostra, para cada dataset, apenas a projeção bidimensional $(X_1,X_2)$ como comparação representativa entre dados reais, Gaussiana única e GMM. Esta seção reproduz a matriz completa de projeções par a par entre todos os atributos, nos três braços, para os quatro cenários principais. Como no corpo do texto, essas figuras são inspeções exploratórias, não testes formais de aderência nem provas de preservação do perfil de cooperação preditiva (Seção~\ref{sec:provenance}).
