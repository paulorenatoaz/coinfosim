...
\begin{tikzpicture}[node distance=5mm and 7mm]
  ...
  \node[bloco, minimum width=15.2cm] (reserv)
    {\textbf{Dados reais de treinamento} (parâmetros de padronização estimados somente aqui)};
  ...
  \node[bloco, below=of perdas, minimum width=15.2cm] (metricas)
    {\textbf{Indicadores de preservação do perfil:} correlação de ranking \; $\cdot$ \; \textit{Winner Agreement} \; $\cdot$ \; concordância de existência de inversões \; $\cdot$ \; similaridade dos tamanhos amostrais};
