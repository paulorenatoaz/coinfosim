% Readable scenario-specific provenance projection — UCI Air Quality
% (scenario 000005). This is an editorial readability adaptation of the
...
% (see coinfosim-report-latex/figures/provenance/provenance_air_quality_000005.pdf
...
% dataset-specific facts (sentinel handling, chronological split, target
% definition) documented in data/raw/air_quality/README.md and
...
\begin{tikzpicture}[node distance=4mm and 4mm]
  ...
  \node[entidade,minimum width=2.8cm]  (source)   at (-3.4, 1.3) {\textbf{SourceDataset}\\UCI Air Quality};
  ...
  \node[nota, text width=3.4cm, above=2mm of trainres] {80\% inicial da coorte cronológica: reserva de treino};
  \node[nota, text width=3.6cm, below=2mm of fixedtest] {20\% final: teste real futuro fixo, não um sorteio aleatório};
  \node[nota, text width=4.6cm, below=2mm of dataprep] {sentinela $-200\to$ ausente; casos completos nos 5 sensores + \texttt{C6H6(GT)}; alvo \texttt{benzene\_elevated} = percentil 75 de \texttt{C6H6(GT)} no treino; padronização só no treino};
