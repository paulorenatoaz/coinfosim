...
\begin{tikzpicture}[node distance=5mm and 5mm]
  ...
  \node[entidade]  (prepared) at (5.2, 18.7) {\textbf{PreparedDataset}\\dataset preparado (alvo, partição, padronização)};
  ...
  % DatasetPreparation is the sole activity that generates three sibling
  % entities (PreparedDataset, TrainingReservoir, FixedRealTestSet); none of
  % those entities generates another, per provenance.provn.
  \draw[relacao] (source)   -- node[rlabel]{used} (dataprep);
  \draw[relacao] (dataprep.east) -- node[rlabel,above]{wasGeneratedBy} (prepared.west);
  \draw[relacaofina] (dataprep.south) -- ++(0,-1.6) coordinate (dpbus) -| node[pos=0.24,rlabel]{wasGeneratedBy} (trainres.north);
  \draw[relacaofina] (dpbus) -| node[pos=0.24,rlabel]{wasGeneratedBy} (fixedtest.north);
