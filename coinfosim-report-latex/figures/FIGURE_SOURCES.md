...
## provenance/ — família de diagramas de proveniência (Capítulo 3 e Apêndice E)

Desde a revisão editorial 0.2.2, as figuras **principais** de proveniência no corpo do relatório são quatro diagramas TikZ legíveis — um modelo genérico (nível 1) e três instanciações por cenário (nível 2) — não mais os grafos Graphviz brutos. Os grafos Graphviz continuam sendo o artefato formal de auditoria (nível 3), preservados e referenciados a partir do Apêndice~E, mas deixaram de ser a figura de exibição principal por serem excessivamente compactos/horizontais para leitura direta no corpo do texto.

| Destino | Fonte | Método |
|---|---|---|
| `provenance/prov_profile_derivation.pdf` / `_wide.pdf` (fonte: `provenance/src/prov_profile_derivation.tex` / `_wide.tex`) | `docs/semantics/provenance_mapping.md`; verificado dataset-independente contra `provenance.provn` dos três cenários | diagrama TikZ genérico (nível 1), usando a camada de estilo compartilhada `provenance/src/prov_diagram_style.tex` (`prov:Entity`=amarelo, `prov:Activity`=azul, `prov:SoftwareAgent`=laranja); versão retrato para o relatório, versão larga 16:9 para a apresentação; projeção de legibilidade, não saída de `prov_to_dot` |
| `provenance/occupancy_provenance.pdf` (fonte: `provenance/src/occupancy_provenance.tex`) | `output/reports/scenarios/000002_occupancy_baseline_full/provenance.provn`; `data/raw/occupancy/README.md`; `chapters/04_datasets.tex` | diagrama TikZ específico do cenário Occupancy (nível 2); mesma camada de estilo; notas tracejadas anotam fatos específicos do dataset (papel de `datatraining.txt`/`datatest.txt`+`datatest2.txt`, padronização só no treino) sem inventar nós PROV |
| `provenance/air_quality_provenance.pdf` (fonte: `provenance/src/air_quality_provenance.tex`) | `output/reports/scenarios/000005_air_quality_baseline_full/provenance.provn`; `data/raw/air_quality/README.md`; `chapters/04_datasets.tex` | idem, cenário Air Quality (sentinela, divisão cronológica, limiar do alvo) |
| `provenance/support2_provenance.pdf` (fonte: `provenance/src/support2_provenance.tex`) | `output/reports/scenarios/000008_support2_baseline_full/provenance.provn`; `chapters/04_datasets.tex` | idem, cenário SUPPORT2, incluindo o nó `RandomForestCalibrationArtifact` (`used`, não gerado, pelas três atividades de simulação, conforme `provenance.provn`) |
| `provenance/provenance_occupancy_000002.pdf`/`.png` | `output/reports/scenarios/000002_occupancy_baseline_full/{scenario.json,semantic_manifest.json}` e resultados persistidos das simulações 000006--000008 | gerado por `coinfosim-report-latex/scripts/generate_scientific_provenance_figures.py`: evidência persistida normalizada em `ProvenanceEvidence` (`collect_persisted_provenance_evidence`), projeção não mutante (`dataclasses.replace`) que limpa somente os campos de engenharia (revisão de código, ambiente de execução, commit de recuperação), `build_scenario_prov_document` canônico, renderizado com `prov.dot.prov_to_dot`/Graphviz; artefato de auditoria (nível 3), referenciado a partir do Apêndice~E, não mais a figura principal do corpo do texto |
| `provenance/provenance_air_quality_000005.pdf`/`.png` | `output/reports/scenarios/000005_air_quality_baseline_full/{scenario.json,semantic_manifest.json}` e resultados persistidos das simulações 000015--000017 | idem, cenário 000005 |
| `provenance/provenance_support2_000008.pdf`/`.png` | `output/reports/scenarios/000008_support2_baseline_full/{scenario.json,semantic_manifest.json}` e resultados persistidos das simulações 000024--000026 | idem, cenário 000008 |

Compilação dos grafos Graphviz: `python coinfosim-report-latex/scripts/generate_scientific_provenance_figures.py`, a partir do ambiente do projeto (requer o executável `dot`).

Compilação dos diagramas TikZ: `xelatex <arquivo>.tex` a partir de `provenance/src/`, reutilizando `figures/conceptual/src/preamble_diagrams.tex` via `prov_diagram_style.tex`; determinística, sem dependência de Graphviz.
