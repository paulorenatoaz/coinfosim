...
O desenho do \CoInfoSim{} adota a mesma direção geral, mas amplia o objeto avaliado. O reservatório real de treinamento é usado para parametrizar um modelo gerador condicional à classe. Esse gerador produz as amostras usadas para treinar os classificadores, enquanto a avaliação ocorre sobre um conjunto real fixo que não participa da parametrização. Para cada dataset são comparados três braços:
\begin{align}
...
No primeiro braço, o treinamento também utiliza amostras reais balanceadas retiradas do reservatório. Nos demais, o treinamento dos classificadores utiliza amostras artificiais produzidas por modelos geradores parametrizados exclusivamente a partir do mesmo reservatório. O teste é compartilhado pelos três braços. Essa escolha reduz uma fonte de variação: as diferenças observadas não decorrem de conjuntos de avaliação distintos.
...
\begin{description}
  ...
  \item[Fidelidade preditiva:] proximidade das perdas absolutas obtidas por treinamento real e sintético;
  \item[Fidelidade estrutural:] preservação da ordenação, das relações de vitória e dos limiares cooperativos entre subconjuntos.
\end{description}
...
\section{Estrutura de cooperação}

A estrutura de cooperação é definida a partir das curvas de perda dos subconjuntos. Para cada $n$, a ordenação das perdas produz um ranking. Comparações par a par identificam qual subconjunto vence cada outro no prefixo analisado. Os cruzamentos das curvas permitem estimar limiares $\Nstar$, que representam transições de superioridade ao longo do tamanho amostral.

Esses objetos possuem níveis diferentes de sensibilidade. Uma correlação de ranking elevada pode coexistir com uma discordância em posições adjacentes. Uma relação de vitória pode mudar por uma diferença pequena de perda. O valor de $\Nstar$ pode se deslocar mesmo quando as curvas mantêm forma geral semelhante, porque depende da posição do cruzamento. Em curvas irregulares, um mesmo par pode cruzar várias vezes; o protocolo atual registra o último cruzamento observado, interpretado como a última transição de superioridade dentro da grade experimental.

A preservação estrutural é, portanto, multidimensional. O trabalho não procura declarar que um braço ``preservou'' ou ``não preservou'' por meio de uma única medida. Em vez disso, compara:

\begin{enumerate}
  \item a ordenação global dos subconjuntos;
  \item as relações de vitória entre pares;
  \item a dinâmica amostral expressa por $\Nstar$.
\end{enumerate}
...
\end{equation}
Nos cenários ancorados, a Gaussiana de cada classe é parametrizada no espaço padronizado a partir do reservatório de treinamento: $\widehat{\mu}_c$ e $\widehat{\Sigma}_c$ correspondem, respectivamente, à média e à covariância amostrais calculadas nesse reservatório. As matrizes de covariância podem diferir entre classes.
...
No protocolo implementado, um GMM é parametrizado separadamente para cada classe no espaço completo dos canais, usando apenas o reservatório real de treinamento. A parametrização de cada candidato é obtida por máxima verossimilhança, e o número de componentes é selecionado pelo menor BIC. Esse critério combina a log-verossimilhança maximizada com uma penalização pela quantidade de parâmetros \cite{schwarz1978}, buscando evitar que a flexibilidade cresça sem controle.

O GMM pode aproximar formas visuais que uma única elipse gaussiana não representa. Ainda assim, maior fidelidade geométrica não garante maior fidelidade estrutural. A divisão dos dados em componentes pode melhorar a representação global da densidade sem preservar exatamente as regiões que determinam as fronteiras de um classificador ou as diferenças relativas entre subconjuntos. Essa possibilidade será importante no Occupancy, em que o GMM parece reproduzir melhor a forma dos dados, mas a Gaussiana única apresenta indicadores estruturais ligeiramente superiores em determinadas configurações finais.
...
Por essa razão, o Random Forest não integra uma comparação simétrica entre todos os datasets nesta versão do trabalho. Ele foi incorporado como estudo adicional no SUPPORT2, com calibração realizada apenas no reservatório real de treinamento e configuração congelada para todos os braços. O resultado deve ser interpretado como evidência específica da interação entre esse classificador, o SUPPORT2 e os geradores avaliados, e não como demonstração de superioridade geral.
...
A motivação de custos permanece, contudo, cientificamente conectada às métricas estruturais. Um ranking preservado permite comparar famílias de configurações; a \WinnersAgreement{} resume relações par a par relevantes para substituir ou eliminar alternativas; e $\Nstar$ indica em que regime amostral uma configuração mais complexa passa a justificar seu uso. Essas informações precedem qualquer otimização monetária e ajudam a formular o problema de decisão adequado a cada domínio.
