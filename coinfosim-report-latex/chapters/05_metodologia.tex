...
Para cada canal $j$, a média $\widehat{\mu}_j$ e o desvio-padrão $\widehat{\sigma}_j$ são calculados somente no reservatório de treinamento. As observações são transformadas por
\begin{equation}
...
\begin{enumerate}
  ...
  \item determinação da coorte elegível;
  \item construção do reservatório de treinamento e do teste real fixo;
...
No braço \RealToReal, amostras balanceadas são retiradas do reservatório real padronizado. Nos dois braços sintéticos, modelos geradores parametrizados a partir desse mesmo reservatório produzem as amostras usadas para treinar os classificadores. Em todos os casos, a avaliação ocorre no teste real fixo. Essa assimetria é intencional: o gerador não é avaliado por produzir um teste artificial fácil para si próprio, mas por sustentar classificadores capazes de reproduzir relações que emergem quando o treinamento usa dados reais. O desenho é relacionado ao protocolo \textit{Train on Synthetic, Test on Real} \cite{esteban2017}, porém o objeto principal aqui não é apenas a utilidade preditiva global, e sim a estrutura de cooperação entre subconjuntos.

A Figura~\ref{fig:conceito-pipeline} resume o fluxo completo. Os três braços partem do mesmo reservatório real de treinamento, divergem apenas na origem das amostras de treinamento e convergem novamente na avaliação, realizada sempre sobre o mesmo conjunto de teste real fixo. As perdas resultantes alimentam as métricas estruturais definidas no Capítulo~6.
...
\end{equation}
em que $\widehat{\bm\mu}_c$ e $\widehat{\bm\Sigma}_c$ são, respectivamente, a média e a covariância amostrais calculadas no espaço completo dos canais, utilizando apenas o reservatório de treinamento. As covariâncias podem diferir entre classes. Quando necessário para garantir definição positiva, é adicionada uma pequena regularização diagonal, registrada nos metadados.
...
A comparação entre os geradores não é formulada como um campeonato no qual o GMM deveria necessariamente vencer. Uma única Gaussiana é computacionalmente mais simples e mais interpretável. O ganho do modelo mais flexível só é relevante quando o treinamento com as amostras que ele produz modifica de modo substantivo a fidelidade estrutural.
...
O Random Forest do SUPPORT2 foi calibrado uma única vez usando exclusivamente o reservatório real de treinamento. A configuração aprovada é armazenada em um artefato versionado e validada por hash do arquivo bruto, impressão digital da partição, definição do alvo, ordem dos canais, semente do particionamento, versão do \textit{scikit-learn} e parâmetros do estimador.
...
A calibração não é executada automaticamente durante um cenário, e não há configuração alternativa silenciosa quando o artefato está ausente ou incompatível. Essa separação impede que o teste real ou cada braço sintético conduza a uma configuração diferente, o que inviabilizaria a comparação estrutural.
