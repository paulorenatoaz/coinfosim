...
\begin{figure}[H]
  ...
  \notailustracao{Esta execução é de escala \textit{smoke} ($n\leq32$, sete subconjuntos de referência) e serve apenas como exemplo didático de leitura conjunta de $\mathbf{W}$ e $\mathbf{R}$. As matrizes de escala completa ($n\leq512$, 127 subconjuntos) publicadas nas versões anteriores deste relatório não puderam ser regeneradas localmente sob o esquema atual porque os resultados brutos dessa execução não estão persistidos neste repositório (Apêndice~B); os resultados numéricos de escala completa para o Random Forest no SUPPORT2 são reportados como indisponíveis no Capítulo~7}
\end{figure}
\end{landscape}
