...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{2.45cm}XXX@{}}
    ...
    \addlinespace
    Dados desta revisão & Resultados brutos das execuções \textit{full} anteriores não estão persistidos localmente. & $\AR$, $\DR$ e $\SR$ são indisponíveis para Occupancy, Air Quality e SUPPORT2 em escala completa nesta edição. & Regenerar ou repersistir os resultados brutos dessas execuções (Apêndice~B). \\
    \bottomrule
  \end{tabularx}
...
Os resultados tampouco se transferem automaticamente entre domínios. Nem a elevada preservação herdada de $\rankrho$/$\AW$ do Gaussian Naive Bayes no Air Quality, nem o padrão de $\AR$/$\SR$ observado no exemplo em escala reduzida do Random Forest no SUPPORT2 (Seção~\ref{sec:support2-rf}), autorizam generalizações para outros conjuntos de sensores ou classificadores. As conclusões constituem evidência de viabilidade em alguns cenários, não uma validação geral: a intensidade e a utilidade da preservação devem ser novamente demonstradas em cada aplicação.

\section{Disponibilidade dos dados brutos sob o esquema atual}

Esta revisão migrou a definição científica do arcabouço retirado de $\NstarHistorical$ para o esquema $\Wmat/\Rmat$ (Capítulo~6), mas não pôde recalcular $\AR$, $\DR$ e $\SR$ para Occupancy, Air Quality e para a escala completa do SUPPORT2, porque os resultados brutos por replicação dessas execuções não estão persistidos neste repositório (Seção~\ref{sec:resultados-guia}; Apêndice~B). Essa é uma limitação da \textbf{disponibilidade de dados nesta revisão editorial}, não do esquema $\Wmat/\Rmat$ em si: o código atual (\texttt{coinfosim.results.structural}) calcula corretamente os quatro indicadores a partir de resultados brutos persistidos, como demonstrado pelo exemplo do SUPPORT2 em escala \textit{smoke} (Seção~\ref{sec:support2-rf}). A caracterização completa do perfil de cooperação preditiva nos três datasets, na escala anteriormente publicada, permanece como trabalho imediato de regeneração, não como questão em aberto sobre a validade do esquema.
