...
Os resultados tampouco se transferem automaticamente entre domínios. Nem a elevada preservação dos quatro indicadores do Gaussian Naive Bayes no Air Quality, nem o padrão de $\AR$/$\SR$ observado no Random Forest no SUPPORT2, em escala completa e no exemplo em escala reduzida (Seção~\ref{sec:support2-rf}), autorizam generalizações para outros conjuntos de sensores ou classificadores. As conclusões constituem evidência de viabilidade em alguns cenários, não uma validação geral: a intensidade e a utilidade da preservação devem ser novamente demonstradas em cada aplicação.

\section{Disponibilidade dos dados brutos sob o esquema atual}

Uma revisão editorial anterior desta migração científica --- do arcabouço retirado de $\NstarHistorical$ para o esquema $\Wmat/\Rmat$ (Capítulo~6) --- não pôde recalcular $\AR$, $\DR$ e $\SR$ para Occupancy, Air Quality e para a escala completa do SUPPORT2, porque os resultados brutos por replicação dessas execuções não estavam persistidos localmente naquele momento. Essa era uma limitação da \textbf{disponibilidade de dados}, não do esquema $\Wmat/\Rmat$ em si: o código (\texttt{coinfosim.results.predictive\_profile}) calcula corretamente os quatro indicadores a partir de resultados brutos persistidos, como já demonstrava o exemplo do SUPPORT2 em escala \textit{smoke} (Seção~\ref{sec:support2-rf}).

Essa limitação foi resolvida nesta revisão: os resultados brutos por replicação das execuções \textit{full} de Occupancy (000002), Air Quality (000005) e SUPPORT2 (000007, 000008) foram recuperados e regenerados, e os quatro indicadores foram recalculados diretamente pelo código atual para os quatro cenários (Capítulo~7). A caracterização completa do perfil de cooperação preditiva nos três datasets, na escala anteriormente publicada, deixa de ser um trabalho pendente. As limitações que permanecem em aberto --- particionamento fixo, incerteza Monte Carlo, balanceamento de classes, número de datasets e classificadores, e os limites de extrapolação além da grade avaliada --- são as discutidas nas seções anteriores deste capítulo, e não decorrem de indisponibilidade de dados.
