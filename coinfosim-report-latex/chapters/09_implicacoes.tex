...
\section{Da preservação estrutural à configuração de canais}
...
A similaridade de \Nstar{} acrescenta outra dimensão. Ela informa se o modelo sintético recupera aproximadamente o regime em que uma configuração passa a superar outra de forma final dentro da grade observada. Essa informação é relevante para planejamento de coleta: um canal adicional pode não ser vantajoso em $n$ pequeno, mas tornar-se útil quando a operação acumula dados suficientes. O uso operacional de \Nstar{}, contudo, exige a cautela discutida nos capítulos anteriores: a métrica considera o último cruzamento observado e não garante que a relação permanecerá inalterada além de $n=512$.
...
O presente trabalho não propõe uma função universal de custo nem demonstra que os subconjuntos identificados gerem economia. Sua contribuição é anterior: organizar a estrutura de desempenho à qual uma função de custo específica poderá ser acoplada.
...
\begin{enumerate}
  ...
  \item os limiares de \Nstar{} usados para planejar expansão amostral.
\end{enumerate}

Essa formulação também permite reconhecer que diferentes decisões exigem diferentes métricas. Para escolher entre várias configurações de custo semelhante, ranking e Winners Agreement podem ser prioritários. Para planejar quando instalar um canal adicional ou ampliar a coleta, \Nstar{} pode ser mais informativo. Não há necessidade de condensar todas essas propriedades em um índice único.
...
A agenda futura consiste, portanto, em transformar o \CoInfoSim{} de um simulador de preservação estrutural em uma plataforma de análise custo-eficiência. A extensão deverá receber custos explícitos, identificar fronteiras não dominadas, quantificar incerteza econômica e testar projeções para regimes maiores. As conclusões atuais permanecem metodológicas: classificadores treinados com amostras sintéticas podem preservar componentes relevantes da cooperação, mas nenhuma economia ou decisão operacional específica foi demonstrada.
